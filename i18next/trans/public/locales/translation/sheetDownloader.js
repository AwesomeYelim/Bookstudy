...
const langs = [require('../ko/translation.json'), require('../ja/translation.json')]; // :ADD 언어 추가시
...
async function Downloader() {
  ...
  headers.forEach((_, i) => {
    const sheetLang = {};
    let jsonLang = {};
    const [strLang, objLang] = [trimHeader[i], langArr[i]];
    rows.forEach((row) => {
    ...
    if ((addKeys && addKeys.length) || (differValues && differValues.length)) {
      ...
      fs.writeFile(
        `src/locales/${strLang}/translation.json`,
